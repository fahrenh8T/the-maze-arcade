...
void render_floor(int bottomwall_pixel, colour_t *texture_colour, int x)
{
	...
	for (y = bottomwall_pixel - 1; y < SCREEN_HEIGHT; y++)
	{
		...
		o = (int)(abs(o * texture_height / 30) % texture_height);

		*texture_colour = wall_textures[4].texture_buffer[(texture_width * o) + textureoffsetX];
...
void render_ceil(int topwall_pixel, colour_t texture_colour, int x)
{
	...
	for (y = 0; y < topwall_pixel; y++)
	{
		...
		o = (int)(abs(o * texture_height / 40) % texture_height);

		texture_colour = wall_textures[6].texture_buffer[(texture_width * o) + textureoffsetX];
		drawPixel(x, y, texture_colour);
	}
}
...
void render_wall(void)
{
	...
	for (x = 0; x < NUM_RAYS; x++)
	{
		...
		for (y = topwall_pixel; y < wallBottomPixel; y++)
		{
			...
			texture_colour = wall_textures[texNum].texture_buffer[(texture_width * o) + textureoffsetX];
			if (rays[x].wasHitVertical)
